...
def overview():
    ...
    if not exists:
        flash(f'Ajouter un produit, ou un emplacement puis faite un transfert', 'info')
...
def product():
    ...
    if exists == False and request.method == 'GET':
        flash(f'Ajouter un produit!', 'info')
        ...
        product.save_to_es()
        logging.debug(f"Produit(s) ajouté(s): {product}")

        flash(f'Le produit {form.prodname.data} a été ajouté!', 'success')
        es.indices.refresh(index='products') # Rafraîchir l'index
        return redirect('/Product')
...
def loc():
    ...
    exists = bool(details)
    if exists == False and request.method == 'GET':
        flash(f'Ajouter un emplacement !', 'info')
    ...
    if lform.validate_on_submit() and request.method == 'POST':
        ...
        flash(f'Emplacement mis à jour !', 'success')
        ...
        flash(f'L\'emplacement {form.locname.data} a été ajouté!', 'success')
        return redirect('/Location')
...
def move():
    ...
    exists = bool(details)
    if exists == False and request.method == 'GET':
        flash(f'Faites un transfert de produit', 'info')
    ...
    src_list_names, dest_list_names = [('Principal', 'Principal')], [('Principal', 'Principal')]
    ...
    if form.validate_on_submit() and request.method == 'POST':
        ...
        if boolbeans == False:
            flash(f'Réessayer avec une quantité inférieur à la source', 'danger')
        elif boolbeans == 'same':
            flash(f'Source et destination ne peuvent pas être identiques.', 'danger')
        elif boolbeans == 'no prod':
            flash(f'Quantité de produit insuffisante. Ajouter des produits', 'danger')
        else:
            ...
            flash(f'Activité ajoutée !', 'success')
...
def check(frm,to,name,qty):
    if frm == to :
        a = 'same'
        return a
    elif frm =='Principal' and to != 'Principal':
        logging.debug(f"Produit à transférer: {name}")
        prodq = es.search(index='products', body={'query': {'match': {'prod_name': name}}})
        prod_qty = prodq['hits']['hits'][0]['_source']['prod_qty']
        
        logging.debug(f"Informations produit transféré : {prod_qty}")
    ...
    elif to == 'Principal' and frm != 'Principal':
        bal = es.search(index='balances', body={'query': {'bool': {'must': [
            {'match': {'location': frm}},
            {'match': {'product': name}}
        ]}}})
...
def delete():
    ...
    if type == 'product':
        ...
        flash(f'L\'emplacement a été supprimé !', 'success')
